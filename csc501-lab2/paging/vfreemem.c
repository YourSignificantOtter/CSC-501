...
int vmcheck_next_block_free(vm_ptr_t *currBlock);
...
SYSCALL	vfreemem(struct mblock *block, unsigned size)
{
	...
	vm_ptr_t *ptr = (vm_ptr_t *)block;
	ptr = ptr - sizeof(vm_ptr_t); //shift back up to where the overhead pointer SHOULD be
	...
	if(size != ptr->vm_size && ptr->vm_size - size < sizeof(vm_ptr_t) + 1) //+1 because need at least 1 byte for the new overhead pointer to point at
	{
		...
		kprintf("Remaining bytes if free occurs: %d\tsizeof(vm_ptr_t): %d\n", ptr->vm_size - size, sizeof(vm_ptr_t));
	...
	if(size == ptr->vm_size && concat == 0) //freeing the entire block but not continuous portion
	{
		...
		ptr->vm_magic_next = ptr + sizeof(vm_ptr_t) + ptr->vm_size; //Point to the next block
	}
	...
	else //Partially freeing a block
	{
		...
		vm_ptr_t *newOH = (vm_ptr_t *)(ptr + sizeof(vm_ptr_t) + ptr->vm_size - size);
		newOH->vm_size = ptr->vm_size - size - sizeof(vm_ptr_t);
...
int vmcheck_next_block_free(vm_ptr_t *currBlock)
{
	...
	vm_ptr_t *test = (vm_ptr_t *)(currBlock + currBlock->vm_size + sizeof(vm_ptr_t));
	...
	else if(test->vm_magic_next == 0)
	{
		...
		ret = -1 * ((proctab[currpid].vhpnpages * NBPG) / (sizeof(vm_ptr_t) + 1));
	}
